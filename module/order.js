...
exports.createOrder = (req, res) => {
    ...
    helper.findShopowner(shopDetail, function(cb) {
        ...
        // console.log('CurrentStoreDetail');
        var DateTime = new Date(req.body.timeForPickcup);
        console.log('timeForPickcup................... ', DateTime);
        // console.log('get hours.......',DateTime.getHours())
        var timeForPickcup = DateTime.getDay();
        // console.log('day************ ', timeForPickcup);
        var getdate = req.body.timeForPickcup.split(" ");
        var startTime = CurrentStoreDetail.shoptime[timeForPickcup].start_time;
        var endTime = CurrentStoreDetail.shoptime[timeForPickcup].end_time;
        // console.log(getdate[0]+" "+(startTime));
        // console.log('shoptime-start_time**-*-**-*-*',Date.parse (getdate[0]+" "+(startTime) ));
        // console.log(Date.parse ( DateTime ) >=  Date.parse (getdate[0]+" "+(startTime) ));
        // console.log(Date.parse ( DateTime ) <= Date.parse (getdate[0]+" "+(endTime) ));
        var newdate = new Date();
        console.log('newdate', newdate)
    if(Date.parse (newdate) >= Date.parse (getdate[0]+" "+(startTime)) && Date.parse (newdate) <= Date.parse (getdate[0]+" "+(endTime)) || 
    Date.parse (DateTime) >= Date.parse (getdate[0]+" "+(startTime)) && Date.parse (DateTime) <= Date.parse (getdate[0]+" "+(endTime))) {
    // helper.ordersheduler(DateTime, (cb) =>{
    //     console.log(cb,'................')
    // })
    helper.findUser(decoded.user._id, function(cb) {
        CurrentUserDetail = cb;
     ...
     //  console.log('inside rewards');
        ...
        reward
         ...
         // console.log("orderType ");normal order
        ...
        var Schedule_name = randomstring.generate({
            length: 9,
            charset: 'hex'
         });
        //console.log(timeForPickcup);
 ...
 ///
         ...
         // console.log('unknoworder')
         // console.log('timeForPickcup', req.body.timeForPickcup);
        //var token = req.body.userToken;
        //var decoded = jwt.decode(token, "pickup");
        var shopDetail = req.body.shopDetail;
        var timeForPickcup = req.body.timeForPickcup;
           ...
           .exec(function(err, reward) {
              ...
              if (!reward) {
                ...
                for(i in reward)
                {
                        ...
                        console.log(startdateData);
                        console.log(dateData);
                        console.log(startdateData <= dateData);
                        console.log(enddateData >= dateData);
                        if ((enddateData >= dateData) && (startdateData <= dateData)) {
                                    ...
                                    usersReward
                                       ...
                                       .exec(function(err, usersRewarddata) {
                                          ...
                                          console.log('usersRewarddata', usersRewarddata);
                                          console.log('err', err);
                                          if (usersRewarddata.claimedReward) {
                                            ...
                                            //    console.log('CurrentStoreDetail.deviceToken', CurrentStoreDetail.deviceToken)
                                                // helper.orderscheduler(timeForPickcup,Schedule_name,orderId,CurrentStoreDetail.deviceToken, (cb) =>{
                                                //     console.log(cb,'................')
                                                // })
                ...
                return res.status(200).json({
                    title: 'Cannot create Order. The Shop is closed for the selected time',
                    error: "false"
                 });
            }
...
//console.log(req.body);
   ...
   tempOrder
      ...
      .populate('userDetail', 'cardDetails')
      .populate('shopDetail', 'status imageurl cafe_name status rating')   // shoptimming
      .exec(function(err, user) {
         if (err) {
            return res.status(500).json({
               title: 'An error occurred',
